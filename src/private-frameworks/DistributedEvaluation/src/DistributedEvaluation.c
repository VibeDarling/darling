...
/* Stub. As of 2026-09-20, these binaries link DistributedEvaluation but bind no symbol from it, so an
...
static void DistributedEvaluation_stub_loaded(void)
{
	if (getenv("STUB_VERBOSE"))
		fprintf(stderr, "STUB: DistributedEvaluation loaded (no symbols implemented)\n");
}

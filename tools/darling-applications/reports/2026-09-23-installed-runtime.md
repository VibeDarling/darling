...
- Foundation [PR #53](https://github.com/VibeDarling/darling-foundation/pull/53) now also imports `NSBundle` as Swift `Bundle` and marks the main bundle nonoptional. Draft [darling-swift PR #54](https://github.com/VibeDarling/darling-swift/pull/54) adopts that import in the Swift overlay and retains localization fallback for an absent lookup. The rebuilt full arm64 Foundation overlay retains all 5,088 prior exported names; a focused `Bundle.main` and `NSLocalizedString` test exits zero in the combined guest prefix. The target-aware `OpenSwiftUICore` build no longer reports missing `Bundle` type errors, but still stops on other APIs, including [CTGlyphInfo issue #833](https://github.com/VibeDarling/darling/issues/833), attributed-string members, and format styles. These fixes change Darling and its Swift library only; Homebrew and app bundles remain untouched.

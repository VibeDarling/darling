...
- Foundation PRs [#53](https://github.com/VibeDarling/darling-foundation/pull/53), [#54](https://github.com/VibeDarling/darling-foundation/pull/54), [#55](https://github.com/VibeDarling/darling-foundation/pull/55), and [#56](https://github.com/VibeDarling/darling-foundation/pull/56) now supply the measured Swift class names, borrowed run-loop ownership, notification bridge, and operation API. These are committed in the local Foundation integration branch. Draft [darling-swift PR #52](https://github.com/VibeDarling/darling-swift/pull/52) adapts the open source `URLRequest` and `URLError` value types. A smaller arm64 Foundation overlay with those types builds and links, and its guest test passes method/header copy-on-write, Objective-C round-trip, and URL error code/domain in a separate prefix. The target-aware dependency build no longer reports missing `URLRequest` or `URLError`; it now exposes absent JSON/property-list encoder types in that smaller overlay and a `URLSession.dataTask` request bridge. The new `NSURLRequest` SwiftBridge note in Foundation PR #53 makes a focused `URLSession.dataTask(with: URLRequest)` arm64 probe typecheck. [Darling issue #831](https://github.com/VibeDarling/darling/issues/831) tracks source evidence that CFNetwork's request copy drops other fields, which the guest test has not yet covered. The complete current Swift overlay, an actual guest URLSession request, Combine.framework, SwiftUI.framework, and AppZapper runtime remain unverified.
...
3. Build and test the complete current Foundation Swift overlay with the URL types and URLSession bridge, then continue the OpenSwiftUI/AttributeGraph chain toward a genuine arm64 Combine and SwiftUI runtime. Rescan AppZapper after each integration step.
